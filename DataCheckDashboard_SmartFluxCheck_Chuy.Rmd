...
flux.files2 <- list.files(path="D:/summaries",full.names=TRUE)
...
data1 <- ldply(flux.files2[1:41], read_column_number)
...
#data10 <- ldply(flux.files2[1279:1388],read_column_number)
...
flux.files.read <- data %>%
  filter(colnumber==207)%>%
  mutate(file.path = paste("D:/summaries/",
                          file,".txt",sep=''))
...
# get column names and units from complete summary files
flux.units2.av <- fread( flux.files.read[flux.files.read$colnumber==207,]$file.path[2], sep="\t", dec=".", header=TRUE, skip=0, fill = TRUE)[1,]
...
nrow.207 <- nrow(flux.files.read[flux.files.read$colnumber==207,])
flux.data2.av <- do.call("rbind", lapply(flux.files.read[flux.files.read$colnumber==207,]$file.path[2:nrow.207], header = FALSE, fread, sep="\t", dec=".",skip = 2, fill=TRUE, na.strings="NaN", col.names=colnames(flux.units2.av)))
...
flux.long <- flux.data2 %>%
  select(date, time, co2_flux, LE, H, co2_mole_fraction, h2o_mole_fraction, co2_signal_strength_7500_mean,
         u_rot, v_rot, w_rot, P_RAIN_1_1_1, PPFD_1_1_1, LWOUT_1_1_1, LWIN_1_1_1, RG_1_1_1, RH_1_1_1, RN_1_1_1, SHF_1_1_1, SHF_2_1_1, SHF_3_1_1, SWC_1_1_1, SWC_2_1_1, SWC_3_1_1, SWIN_1_1_1, SWOUT_1_1_1, TA_1_1_1, TS_1_1_1, TS_2_1_1, TS_3_1_1, VIN_1_1_1, ALB_1_1_1, air_pressure) %>%
...
footprint.data <- flux.data2 %>%
  select(date_time,`u*`,`x_90%`,`x_70%`,`x_50%`,`x_30%`,`x_10%`)%>%
  filter(`u*`>0.2) %>%
  drop_na
...
selectInput("variable5", "Soil variable 1:",
             c("All Soil Temperature Profile" = "^TS_1",
               ...
               "Soil Temperature 1 " ="^TS_1_1_1",
               "Soil Temperature 2 " ="^TS_2_1_1",
               "Soil Temperature 3 " ="^TS_3_1_1",
               "Soil Moisture 1" ="^SWC_1_1_1",
               "Soil Moisture 2" ="^SWC_2_1_1",
               "Soil Moisture 3" ="^SWC_3_1_1"))
